...
@Component({
  selector: 'froalatime',
  templateUrl: './froalatime.html'
})
export class FroalaTime {
  dataSource: any;
  type: string;
  width: string;
  height: string;
  ...
  updateMyChartData() {
    let updatedfroalaTable = new FroalaCharts.DataStore().createDataTable(
      updatedDataSource,
      schema
    );
    this.dataSource.caption.text = 'HELLLOOO!!!!';
    this.dataSource.data = updatedfroalaTable;
  }
}
